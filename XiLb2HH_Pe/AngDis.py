...
# Default values of the global parameters:
    ...
    def __init__(self, PaPsi=aPsi, PpPsi=pPsi, PPe=Pe, PaX=aX, PpX=pX, PaL=aL, PaLb=aLb):
        self.aPsi = PaPsi
        self.pPsi = PpPsi
        self.Pe = PPe
        self.aX = PaX
        ...
        self.aL = PaL
        self.aLb = PaLb
        self.DL0 = AADecay12(PaL)
        self.DLb0 = AADecay12(PaLb)
        self.DX = AADecay12(PaX,PpX)
...
#  Print the values of class instance parameters:
    ...
    def calc_pdf(self, tX, tL, pL, tp, pp, tpb, ppb):
    ...
    # 
        ...
        start = time.time()
        V = 2*(4*pi)**3*(3+aPsi)/3.0  # volume
        V1 = -2*(4*pi)**3*(3+aPsi)*(3+aPsi)/3.0  # volume derivative
        pdf = multi_dot([dL0_A[:,0], dX_A, dC_A, dLb0_A[:,0]])/V

        # ---------------------------------   Analytic derivatives calculation      
        
        dapsi = multi_dot([dL0_A[:,0], dX_A, dC_aP, dLb0_A[:,0]])/V + pdf*V/V1
        dppsi = multi_dot([dL0_A[:,0], dX_A, dC_pP, dLb0_A[:,0]])/V
        dpe = multi_dot([dL0_A[:,0], dX_A, dC_Pe, dLb0_A[:,0]])/V
        dal = multi_dot([dL0_aM[:,0], dX_A, dC_A, dLb0_A[:,0]])/V
        dax = multi_dot([dL0_A[:,0], dX_aM, dC_A, dLb0_A[:,0]])/V
        dpx = multi_dot([dL0_A[:,0], dX_pM, dC_A, dLb0_A[:,0]])/V
        dalb = multi_dot([dL0_A[:,0], dX_A, dC_A, dLb0_aM[:,0]])/V
        
        end = time.time()
        self.exec_time_loop_pdf += end-start

        self.dvec = [pdf, dal, dalb, dax, dpx, dapsi, dppsi, dpe]

        return [pdf, dal, dalb, dax, dpx, dapsi, dppsi, dpe]

    def gg(self,x):
        """_summary_
        This is the direct interface to communicate with the vegas package.
        This function is needed for vegas.Integrator()
        In this case it is working with 7-dimensional vector of parameters. 
        Args:
        x (_list_): _7-dimensional point_
        ...
        start = time.time()
        self.calc_pdf(tX, tL, pL, tp, pp, tpb, ppb)
        ...
        jac = sin(tX)*sin(tL)*sin(tp)*sin(tpb)  # Jacobian
